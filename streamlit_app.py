...
# --- Navigation ---
if st.sidebar.toggle("HOME"):
    st.session_state.page = "Home"
elif st.sidebar.toggle("BUSINESS CASE STUDY"):
    st.session_state.page = "Business Case Study"
elif st.sidebar.toggle("CASE STUDY INSIGHTS"):
...
if st.session_state.page == "Home":
    ...
    st.markdown("""
        ...
        <div class="home-subtitle">Digital Payments Analytics - Transactions, Users & Insurance Trends</div>
    ...
    st.markdown("""
        <div class="info-box">💳 <span class='highlight'>Transactions:</span> Track volumes by type, region, and time.</div>
        <div class="info-box">👥 <span class='highlight'>Users:</span> See how people use the app across brands and states.</div>
        <div class="info-box">🛡 <span class='highlight'>Insurance:</span> View adoption trends across states and districts.</div>
        <div class="info-box">🌐 <span class='highlight'>Visuals:</span> Explore data with maps, bars, lines, and pies.</div>
        <div class="info-box">📊 <span class='highlight'>Filters:</span> Choose year and quarter for custom insights.</div>
        """, unsafe_allow_html=True)
...
# --- Business Case Study ---
    ...
    if sub_tab == "Transaction":
        ...
        st.write("🛡 Insurance Analytics Here...")
    ...
    year = ["All"] + [str(y) for y in range(2018, 2025)]
    quarters = ["All"] + list(quarter_map.keys())

    col1, col2 = st.columns(2)
    selected_year = col1.selectbox("Select Year", year)
    ...
    if sub_tab == "Transaction":
        ...
        df_total = execute_query(f"""
            SELECT SUM(RegisteredUsers) as TotalUsers, SUM(AppOpens) as TotalOpens 
        ...
        with tab1:
            df_states = execute_query(f"""
                SELECT States, SUM(RegisteredUsers) as TotalUsers 
                FROM map_user {where_clause} 
        ...
        with tab2:
            df_districts = execute_query(f"""
                SELECT Districts, SUM(RegisteredUsers) as TotalUsers 
                FROM map_user {where_clause} 
        ...
        with tab3:
            df_pincodes = execute_query(f"""
                SELECT Pincodes, SUM(RegisteredUsers) as TotalUsers 
...
# Case Study Insights
    ...
    if case_option == "Decoding Transaction Dynamics":
        df_type = execute_query(f"SELECT Transaction_type, SUM(Transaction_count) AS TotalCount FROM aggregated_transaction {where_clause} GROUP BY Transaction_type", tuple(params))
        ...
        df_trend = execute_query(f"SELECT Year, SUM(Transaction_amount) AS Amount FROM aggregated_transaction {where_clause} GROUP BY Year", tuple(params))
        st.plotly_chart(px.line(df_trend, x="Year", y="Amount", markers=True, title="Transaction Trend Over Years", color_discrete_sequence=["orange"]))
        ...
        df_users = execute_query(f"SELECT Brands, SUM(Transaction_count) AS Users FROM aggregated_user {where_clause} GROUP BY Brands", tuple(params))
        ...
        df_opens = execute_query(f"""
            ...
            JOIN map_user mu ON au.States = mu.States AND au.Year = mu.Year AND au.Quarter = mu.Quarter
            {where_clause.replace('Year', 'au.Year').replace('Quarter', 'au.Quarter')}
            GROUP BY au.Brands
        """, tuple(params))
        ...
        df_state = execute_query(f"SELECT States, SUM(Transaction_count) AS TotalCount FROM aggregated_insurance {where_clause} GROUP BY States", tuple(params))
        ...
        df_line = execute_query(f"SELECT Year, SUM(Transaction_count) AS Count FROM aggregated_insurance {where_clause} GROUP BY Year", tuple(params))
        if not df_line.empty and "TotalCount" in df_line.columns:
            st.plotly_chart(px.line(df_line, x="Year", y="Count", markers=True, title="Insurance Growth Over Time", color_discrete_sequence=["#EF476F"]))
        else:
            st.warning("No data available .")

    elif case_option == "Transaction Analysis for Market Expansion":
        df_amount = execute_query(f"SELECT States, SUM(Transaction_amount) AS Amount FROM aggregated_transaction {where_clause} GROUP BY States", tuple(params))
        ...
        df_yearwise = execute_query(f"SELECT Year, SUM(Transaction_amount) AS Total FROM aggregated_transaction {where_clause} GROUP BY Year", tuple(params))
        st.plotly_chart(px.scatter(df_yearwise, x="Year", y="Total", title="Transaction Value Over Year", color_discrete_sequence=["#2404C0"]))
        ...
        df_reg = execute_query(f"SELECT Year, SUM(RegisteredUsers) AS Users FROM map_user {where_clause} GROUP BY Year", tuple(params))
        st.plotly_chart(px.line(df_reg, x="Year", y="Users", markers=True, title="User Registrations Over Time", color_discrete_sequence=["#FF6B6B"]))

        df_districts = execute_query(f"SELECT Districts, SUM(RegisteredUsers) AS Users FROM map_user {where_clause} GROUP BY Districts ORDER BY Users DESC LIMIT 10", tuple(params))
